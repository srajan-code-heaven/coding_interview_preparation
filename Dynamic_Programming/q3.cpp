...
int maxCoinsRec(char arr[][C],int dp[R][C][2],int i,int j,int d)
{
    ...
    dp[i][j][d]=arr[i][j]!='E'?1:0;
    if(d==1)
    {
        dp[i][j][d]= dp[i][j][d]+max(maxCoinsRec(arr,dp,i,j+1,1),maxCoinsRec(arr,dp,i+1,j,0));
    }
    else
    {
         dp[i][j][d]=dp[i][j][d]+max(maxCoinsRec(arr,dp,i,j-1,0),maxCoinsRec(arr,dp,i+1,j,1));
    }
    return dp[i][j][d];
}
